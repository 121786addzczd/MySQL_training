...
FROM
  salaries
WHERE employee_id IN(SELECT id FROM employees WHERE department_id = 2);
...
SELECT
CASE
  WHEN cs.id < 5 THEN "クラス1"
  ELSE "クラス2"
END AS "クラス分類",
COUNT(std.id)
FROM students AS std
INNER JOIN enrollments AS en
ON std.id = en.student_id
INNER JOIN classes AS cs
ON en.class_id = cs.id
GROUP BY
CASE
  WHEN cs.id < 5 THEN "クラス1"
  ELSE "クラス2"
...
WHERE od.customer_id = cst.id) AS order_amountの合計値
...
SELECT
  CONCAT(customers.last_name, first_name), COUNT(*)
FROM
...
INNER JOIN (SELECT * FROM stores WHERE stores.name = "山田商店") AS stores
ON items.store_id = stores.id
GROUP BY CONCAT(customers.last_name, first_name)
...
WHERE EXISTS(
SELECT 1 FROM salaries AS sa WHERE emp.id = sa.employee_id AND sa.payment > 9000000
)
